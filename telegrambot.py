...
TOKEN = "5572502225:AAGzyeuzptbYWYAqH_Q1EAi9ES-vvsZvdG8"
...
def tel_send_image(chat_id):
    ...
    payload = {
        'chat_id': chat_id,
        'photo': "https://cdn.discordapp.com/attachments/829508668923314217/977053263117553704/oru407q150zmLUEkEZo-o.jpg"
    }
    r = requests.post(url, json=payload)
    return r
...
def get_profile():
    ...
    url = f'https://api.telegram.org/bot{TOKEN}/getChat?chat_id=1102455203'
    # r = requests.get(url)
    # print("rrrr",r)
    a = requests.post(url)
    json_resp = json.loads(a.content)
    ...
    # g_chat_id = message['message']['chat']['id']
    file_id = json_resp['result']['photo']['big_file_id']
    # print("g_chat_id-->", g_chat_id)
    print("g_image_id-->", file_id)
   
    url = f'https://api.telegram.org/bot{TOKEN}/getFile?file_id={file_id}'
    b = requests.post(url)
    json_resp = json.loads(b.content)
    print("json_resp111-->", json_resp)
    file_pathh = json_resp['result']['file_path']
    print("file_pathh111-->", file_pathh)

    # saving the file to our computer
    url_1 = f'https://api.telegram.org/file/bot{TOKEN}/{file_pathh}'
    c = requests.get(url_1)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if txt == "hi":
                ...
                # chat_id = tel_parse_get_profile(msg)
                get_profile()
